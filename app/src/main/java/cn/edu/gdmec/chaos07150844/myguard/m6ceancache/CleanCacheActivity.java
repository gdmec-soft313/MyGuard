...
public class CleanCacheActivity extends AppCompatActivity implements View.OnClickListener{
    protected static final int CLEANNIG = 100;
    ...
    class ClearCacheObsercer extends android.content.pm.IPackageDataObserver.Stub{
    ...
    private void cleanAll(){
        ...
        for(Method method :methods){
            if("freeStorageAndNotify".equals(method.getName())){
                try {
                    method.invoke(pm,Integer.MAX_VALUE,new ClearCacheObsercer());
                } catch (Exception e) {
                    e.printStackTrace();
                }
                return;
